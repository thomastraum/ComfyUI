...
# Before first run, sync your local models to the volume
@modal.function(volumes={"/models": models_vol})
def sync_models():
    from pathlib import Path
    
    vol = modal.Volume.lookup("comfyui-models")
    local_models = Path(__file__).parent / "models"
    
    if local_models.exists():
        with vol.batch_upload() as batch:
            # Upload the entire directory at once
            batch.put_directory(str(local_models), "/")

# Update the image to symlink the models volume to ComfyUI's model directory
image = (
    # install huggingface_hub with hf_transfer support to speed up downloads
    # image.pip_install("huggingface_hub[hf_transfer]==0.26.2")
    # .env({"HF_HUB_ENABLE_HF_TRANSFER": "1"})
    image.run_commands(
        # Simply link the entire models directory
        "ln -s /models/* /root/comfy/ComfyUI/models/"
    )
)
...
@app.cls(
    ...
    volumes={
        "/cache": vol,
        "/models": models_vol,  # Add the models volume mount
    },
)
class ComfyUI:
    @modal.enter()
