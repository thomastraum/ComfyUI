...
def get_comfyui_version():
    ...
    try:
        ...
        try:
            ...
            comfyui_version = subprocess.check_output(
                ["git", "describe", "--tags"], cwd=repo_path
            ).decode("utf-8")
...
class PromptServer:
    def __init__(self, loop):
        ...
        def image_upload(post, image_save_function=None):
            logging.info("Starting image_upload function")

            image = post.get("image")
            logging.info(f"Image from post: {image}")
            ...
            logging.info(f"Image upload type: {image_upload_type}")

            upload_dir, image_upload_type = get_dir_by_type(image_upload_type)
            logging.info(
                f"Upload directory: {upload_dir}, Image upload type: {image_upload_type}"
            )

            if image and image.file:
                logging.info("Image and image.file exist")
                filename = image.filename
                logging.info(f"Filename: {filename}")

                if not filename:
                    logging.warning("Filename is empty, returning 400")
                ...
                logging.info(f"Full output folder: {full_output_folder}")

                filepath = os.path.abspath(os.path.join(full_output_folder, filename))
                logging.info(f"Filepath: {filepath}")

                if os.path.commonpath((upload_dir, filepath)) != upload_dir:
                    logging.warning("Invalid filepath, returning 400")
                    return web.Response(status=400)

                if not os.path.exists(full_output_folder):
                    logging.info(f"Creating directory: {full_output_folder}")
                    os.makedirs(full_output_folder)

                split = os.path.splitext(filename)

                if overwrite is not None and (overwrite == "true" or overwrite == "1"):
                    logging.info("Overwrite is true")
                else:
                    logging.info("Checking for existing files")
                    i = 1
                    while os.path.exists(filepath):
                        if compare_image_hash(filepath, image):
                            logging.info("Image is a duplicate")
                ...
                logging.warning("No image in post, returning 400")
                return web.Response(status=400)
        ...
        async def download_handler(request):
            ...
            progress_interval = data.get(
                "progress_interval", 1.0
            )  # In seconds, how often to report download progress.

            if not url or not model_directory or not model_filename or not folder_path:
                return web.json_response(
                    {
                        "status": "error",
                        "message": "Missing URL or folder path or filename",
                    },
                    status=400,
                )
